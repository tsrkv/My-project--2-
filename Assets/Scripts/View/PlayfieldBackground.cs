...
public class PlayfieldBackground : MonoBehaviour
{
    ...
    void Ensure()
    {
        ...
        if (_fill == null)
        {
            ...
            // ВАЖНО: используем стандартный материал спрайтов
            _fill.sharedMaterial = null;         // Sprites/Default
            // 1x1 белый спрайт
            _tex1x1 = new Texture2D(1,1,TextureFormat.RGBA32,false);
            _tex1x1.SetPixel(0,0,Color.white); _tex1x1.Apply();
            _fill.sprite = Sprite.Create(_tex1x1, new Rect(0,0,1,1), new Vector2(0.5f,0.5f), 1f);
        }
        if (_border == null)
        {
            ...
            _border.sortingOrder = -150; // поверх заливки, но под сеткой/лучами
        }
    }
